...
```c++
constexpr int group(HttpStatus status) noexcept;
...
Query which response group a status code belongs to. The `group()` function
returns a multiple of 100, equal to the status code with the ones and tens
digits truncated.
